...
public class Originator {
    ...
    protected ArrayList<ArrayList<ArrayList<Integer>>> bdr = new ArrayList<>();
    protected int lpx = -1;
    protected int lpy = -1;
    protected int mRC = 0;
    ...
    public IFile createFile(){
        ...
        return newfactory.createSavedBoard(gameType, SL, mRC, mRCB, mRCW, lpx, lpy, bN, wN, pling, BT, wR, BW, bd ,bdr);
    }
    public void restoreFile(IFile file){
        ...
        this.setBoardRecord(afile.getBoardRecord());
    }
    ...
    public void setBoardRecord(ArrayList<ArrayList<ArrayList<Integer>>> boardRecord){
        ArrayList<ArrayList<ArrayList<Integer>>> tempbdr = new ArrayList<>();
        for(int i=0;i<boardRecord.size();i++){
            ArrayList<ArrayList<Integer>> tempbd = boardRecord.get(i);
            ArrayList<ArrayList<Integer>> tempframe = new ArrayList<>();
            for(int j=0;i<SL;j++){
                tempframe.add((ArrayList<Integer>) tempbd.get(j).clone());
            }
            tempbdr.add(tempframe);
        }
        bdr = tempbdr;
    }
