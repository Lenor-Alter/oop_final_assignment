...
public class BasicBoard implements IFile,Cloneable{
    ...
    protected ArrayList<ArrayList<ArrayList<Integer>>> boardRecord = new ArrayList<>();
    ...
    BasicBoard(int gT, int SL, int mRC, int mRCB, int mRCW, int lpx, int lpy, int bN, int wN, boolean pling, boolean BT, boolean wR, boolean BW,ArrayList<ArrayList<Integer>> bd,ArrayList<ArrayList<ArrayList<Integer>>> bdr){
        gameType = gT;
        SideLength = SL;
...
//            showBoard();
    ...
    void record(){
        ArrayList<ArrayList<Integer>> board_now = new ArrayList<ArrayList<Integer>>();
        for(int i=0;i<SideLength;i++){
            board_now.add((ArrayList<Integer>) board.get(i).clone());
        }
        boardRecord.add(board_now);
    }
    void showRecord(){
        for(int i=0;i<boardRecord.size();i++){
            ArrayList<ArrayList<Integer>> bdFrame = boardRecord.get(i);
            System.out.println("\n第"+i+"步如下：");
            showFrame(bdFrame);
        }
    ...
    void showFrame(ArrayList<ArrayList<Integer>> bdFrame){
        System.out.print("\n    ");
        for(int i=0;i<SideLength;i++){
            if(i<10){
                System.out.print(i+"   ");
        ...
        for(int i=0;i<SideLength;i++){
            ...
            ArrayList<Integer> row = bdFrame.get(i);
            for(int j=0;j<SideLength;j++){
                if(row.get(j) == 0){
                    // 打印黑子
                    System.out.print("X ");
...
//    }
    ...
    ArrayList<ArrayList<ArrayList<Integer>>> getBoardRecord(){   return boardRecord;}
